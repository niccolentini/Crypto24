...
//  int EVP_CipherFinal(EVP_CIPHER_CTX *ctx, unsigned char *outm, int *outl);
    ...
    for(int i = 0; i < strlen(argv[2])/2;i++){
        sscanf(&argv[2][2*i],"%2hhx", &key[i]); // each 2 hexadecimal characters are converted into a byte (input is in hex format)
    }
    ...
    while((n_read = fread(buffer,1,MAX_BUFFER,f_in)) > 0){ // fread specifies in order the buffer, the size of each element, max number of elements to read and the file
