...
def login():
    ...
    if admin != 1:
        ...
        session['admin_expire_date'] = int(time.time()) - randint(10, 266) * 24 * 60 * 60 # 10-266 days ago randomly chosen
    expire_date = int(time.time()) + 30 * 24 * 60 * 60 # expire in 30 days
...
def get_flag():
    ...
    try:
        ...
        if 290 * 24 * 60 * 60 < abs(int(token["expires"]) - session['admin_expire_date']) < 300 * 24 * 60 * 60: # checks if the expire date is between 290 and 300 days ago
